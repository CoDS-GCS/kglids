...
subject_node = '2Table'  # gets overwritten
...
def get_column_emb(graph_name):
    df = pd.read_csv('OnDemandDataPrep/Modeling/storage/Embeddings_columns_' + graph_name + '.csv')
    file1 = pd.read_csv('OnDemandDataPrep/Modeling/storage/' + graph_name + '/mapping/1Column_entidx2name.csv')
...
class RGCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
                 dropout, num_nodes_dict, x_types, num_edge_types, graph_name):
    ...
    def reset_parameters(self):
        for key, emb in self.emb_dict.items():
            if int(key) == 0:
                self.emb_dict[key] = get_column_emb(self.graph_name)
            else:
...
def graphSaint(graph_name):
    def train(epoch):
        ...
        for data in train_loader:
            ...
            y = data.y[data.train_mask].view(-1)
    ...
    start_t = datetime.datetime.now()
    df = pd.read_csv('OnDemandDataPrep/Modeling/storage/Embeddings_tables_' + graph_name + '.csv', index_col=False)
    file1 = pd.read_csv('OnDemandDataPrep/Modeling/storage/' + graph_name + '/mapping/2Table_entidx2name.csv', index_col=False)
    file1['ent name'] = file1['ent name'].astype(str)
    df_emb = pd.merge(file1, df, left_on='ent name', right_on='Key', how='left')
    df_emb = df_emb.drop('ent name', axis=1)
    df_emb = df_emb.drop('ent idx', axis=1)
    ...
    df_emb = df_emb.fillna(0)
    array_emb = df_emb.values
    feat2 = torch.from_numpy(array_emb.astype(np.float32))
    ...
    model = RGCN(1800, args.hidden_channels, dataset.num_classes, args.num_layers,
                 args.dropout, num_nodes_dict, list(x_dict.keys()),
                 len(edge_index_dict.keys()), graph_name).to(device)
    ...
    if args.loadTrainedModel == 1:
        ...
        pd.DataFrame(dic_results).transpose().to_csv("OnDemandDataPrep/Modeling/storage/GSAINT_" +
                                                     GA_dataset_name + "_Times_cleaning.csv", index=False)

        print('model.state_dict()', model.state_dict())
        torch.save(model.state_dict(),
                   "OnDemandDataPrep/Models/" + dataset_name + "_cleaning.model")
