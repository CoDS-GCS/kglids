...
class PygNodePropPredDataset_hsh(InMemoryDataset):
    def __init__(self, name, root = '/',numofClasses=3, transform=None, pre_transform=None, meta_dict = None):
        '''
        ...
        if meta_dict is None:
            ...
            self.original_root = self.datasets_path
        ...
        self.download_name = self.datasets_path + f.namelist()[0].split("/")[0]
        ...
        self.meta_info['split'] = 'random'#'time'
    ...
    def download(self):
        ...
        if str(url).startswith("http")==False:
            ...
            shutil.move(osp.join(self.download_name), self.root)
        else:
