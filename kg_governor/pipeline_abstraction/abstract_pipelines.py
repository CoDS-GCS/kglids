...
from kg_governor.pipeline_abstraction.json_to_rdf import build_pipeline_rdf_page, build_default_rdf_page, build_library_rdf_page
from kg_governor.pipeline_abstraction.util import url_encode
from kglids_config import KGLiDSConfig

def abstract_pipelines():
    start_time = datetime.now()

    pipelines = []
    datasets = list(os.scandir(KGLiDSConfig.data_source_path))
                        ...
                        for file in os.scandir(pipeline.path):
                            if '.py' in file.name:
                                pipelines.append((working_file, dataset.name, file.path, pipeline_info,
                                                  KGLiDSConfig.pipeline_graphs_out_path,
                                                  pipeline_info['id']))
    ...
    with open(os.path.join(KGLiDSConfig.pipeline_graphs_out_path, 'library.ttl'), 'w') as f:
        f.write(build_library_rdf_page(libs))

    with open(os.path.join(KGLiDSConfig.pipeline_graphs_out_path, 'default.ttl'), 'w') as f:
...
def pipeline_analysis(args):
    ...
    SOURCE = KGLiDSConfig.data_source
