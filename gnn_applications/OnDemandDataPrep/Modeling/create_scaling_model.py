...
def graphSaint(graph_name):
    ...
    df = pd.read_csv('OnDemandDataPrep/Modeling/storage/Embeddings_tables_'+ graph_name +'.csv')
    file1 = pd.read_csv('OnDemandDataPrep/Modeling/storage/' + graph_name + '/mapping/2Table_entidx2name.csv')
    file1['ent name'] = file1['ent name'].astype(str)
    df_emb = pd.merge(file1, df, left_on='ent name', right_on='Key', how='left')
    df_emb = df_emb.drop('ent name', axis=1)
    df_emb = df_emb.drop('ent idx', axis=1)
    ...
    pd.DataFrame(dic_results).transpose().to_csv(
        "OnDemandDataPrep/Modeling/storage/GSAINT_" + GA_dataset_name + "_scaling.csv", index=False)
    print('model.state_dict()', model.state_dict())
    torch.save(model.state_dict(), "OnDemandDataPrep/Models/" + dataset_name + "_scaling.model")
